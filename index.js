...
function createRouletteSlice(sliceName, position, sliceCount) {
  ...
  $slice.setAttribute(
    "d",
    `M ${CIRCLE_RADIUS_PLUS_STROKE} ${CIRCLE_RADIUS} L ${CIRCLE_RADIUS_PLUS_STROKE} ${STROKE} A ${CIRCLE_RADIUS_MINUS_STROKE} ${CIRCLE_RADIUS_MINUS_STROKE} 0 0 1 ${arcEndX} ${arcEndY}`
  );
...
$getEle("#spin").addEventListener("click", () => {
  const $roulette = $getEle("#roulette-wrapper");
  ...
  setTimeout(() => {
    ...
    const animationDuration = randomNumber(3.8, 5);
    const finalCoil =
